...
MORPHOLOGY_ANALYSIS_FILENAME = "analysis_llm_morphologic.txt"
SYNTAX_ANALYSIS_FILENAME = "analysis_llm_synthactic.txt"
...
def load_syntax_parses(segment_dir):
    syntax_path = os.path.join(segment_dir, SYNTAX_ANALYSIS_FILENAME)
    if not os.path.exists(syntax_path):
        return []
    try:
        raw = Path(syntax_path).read_text(encoding="utf-8")
    except OSError:
        return []
    payload = parse_json_payload(raw)
    if not payload:
        return []
    return payload.get("syntax", {}).get("parses", [])


def load_key_lemmas(segment_dir):
    morph_path = os.path.join(segment_dir, MORPHOLOGY_ANALYSIS_FILENAME)
    if not os.path.exists(morph_path):
        return []
    try:
        raw = Path(morph_path).read_text(encoding="utf-8")
    except OSError:
        return []
    payload = parse_json_payload(raw)
    if not payload:
...
def write_analysis(target_dir, content):
    target_path = os.path.join(target_dir, "analysis_llm_semantic_historical.txt")
...
def build_prompt(text_content, phase_limit, parses, key_lemmas):
    if parses or key_lemmas:
        payload = {"parses": parses, "key_lemmas": key_lemmas}
        input_block = f"Input JSON:\n{json.dumps(payload, ensure_ascii=False, indent=2)}"
        context_block = f"\n\nSource Text:\n{text_content}"
    else:
        input_block = f"Input Text:\n{text_content}"
        context_block = ""
    return f"""Instruction: Perform a scientific Semantic & Historical Evaluation (Level D) of the Ge'ez text.
Goal: Assess syntactic hypotheses using historical evidence and document uncertainty.

SCIENTIFIC RULES:
1. CONTEXT INVARIANCE: mark whether readings stay stable with added context.
2. ATTESTATION: cite parallels only if actually attested; otherwise mark unattested.
3. DECISION LOG: explain why a reading is preferred; do not hide uncertainty.
4. BACK-TRANSLATION CHECK: flag added concepts not present in the source.

{input_block}{context_block}
        ...
        "segments": [
            {
                ...
                "parses": segment["parses"],
                "key_lemmas": segment["key_lemmas"],
            }
            for segment in segments
        ],
    }
    return (
        "Instruction: Perform a semantic & historical evaluation (Level D) per segment.\n"
...
def collect_segment_inputs(chapter_id, chapter_name, chapter_dir, segment_label, completed, target_chapters, include_wave):
    ...
    for segment_name, segment_dir in iter_segments(chapter_dir, segment_label):
        ...
        if key in completed and not target_chapters:
            continue
        wait_for_dependency(
            os.path.join(segment_dir, SYNTAX_ANALYSIS_FILENAME),
            "synthactic",
        )
        ...
        segments.append({
            ...
            "parses": load_syntax_parses(segment_dir),
...
def main():
    ...
    if not progress_csv:
        progress_csv = str(Path(data_root) / "analysis" / "analysis_progress_python_semantic_historical_v2.csv")
    else:
    ...
    for chapter_name, chapter_num in chapter_entries:
        ...
        if args.per_segment:
            ...
            wait_for_dependency(
                os.path.join(chapter_dir, SYNTAX_ANALYSIS_FILENAME),
                "synthactic",
            )
            ...
            parses = load_syntax_parses(chapter_dir)
