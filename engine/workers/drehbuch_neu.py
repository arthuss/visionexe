...
1. GENRE & VIBE: Industrial Mysticism / Simulation-Forensics. Ancient settings meet mystical technology.
...
4. TRANSITION PROTOCOL (Interpolation):
   ...
   - PORTAL INGRESS: Vertical rift, grid interference, stepping from dust into sterile mainframe.
...
5. ACTOR EVOLUTION (Gear Tracking):
   ...
   - Kap 71-108 (MASTER): White plasma body, voxel-transparency, integrated with the mainframe.
6. VISUAL LAYERS (ABC Integration):
   - LAYER A (FLESH): Bio-detail, sweat, textures, 85mm+ Macro.
   - LAYER B (GRID): Environment blueprints, laser-vectors, 14mm God-Eye.
...
def get_chapter_data(chapter_path):
    data = {}
    ...
    sub_folders = ["analysis_linguistik", "tech_hypothesen", "visual_abc", "einleitung", "integration_wave"]
...
DEINE AUFGABE: Analysiere den alten Entwurf. Wo war er zu plastisch?? Wo fehlte die technische Tiefe?denk dran wir drehen die clips in realismus, die actor werden zwar von loras verändert haben aber photograpfische grundactors, heist der stil muss realistisch bleiben.
Erstelle eine verbesserte VERSION 2.0, die noch präziser dem 'Rule of Mechanism' folgt. vergiss dabei aber nicht das es um ein visuelles konzept handelt  also müssen die bilder im kopf des lesers entstehen  daher darf es nicht zu technisch werden sondern muss die balance zwischen technik und visualisierung halten. fliesende pixel sind vlt nicht der grad an realismus den wir uns vorstellen das sieht meist nach lego movie aus, vermeide  sowas!  liefere das komplette, verbesserte konzept zurück.
'''
...
2. MESH-BREAK: Wie bricht das Mesh der Welt in diesem Moment? (Vertex-Glitch? Thermal Throttling? Fluid Dynamics?)
3. PACING-STRATEGIE: Wie ist der Schnitt-Rhythmus? (Hektische Glitch-Cuts bei Schock? Zeitlupe bei Ingress?)
...
Die Exegese: Das ist Node-Hopping. Die Erde ist ein Gitter (Grid). Er tritt an einem Knotenpunkt (z.B. Sinai) in ein Feld und kommt instantan an einem anderen Knotenpunkt (z.B. Himalaya/Zimt-Tal) wieder heraus.
Visual: Kein Flug. Die Realität um ihn herum reißt auf (Glitch-Effekt) oder verzerrt sich. Er macht einen Schritt in der Wüste und der Fuß landet im Schnee.
...
- Nutze schnelle Schnitte (2-3s) für Action/Glitch.
- Nutze lange Shots (5-8s) für Atmosphäre/Ingress.
- Prüfe am Ende, ob die Gesamtzeit passt.
"""

def build_production_prompt(data, kb, concept_output, script_structure, chapter_num, existing_script=None):
    refinement_instruction = ""
    if existing_script:
        refinement_instruction = f'''
...
def main():
    ...
    data = get_chapter_data(chapter_path)
    ...
    concept_prompt = build_concept_prompt(data, kb, args.chapter, old_concept)
    concept_text = call_ai_agent(concept_prompt, "Visionary Concept Generation")
    ...
    script_structure_prompt = build_script_structure_prompt(data, kb, concept_text, args.chapter)
    script_structure_text = call_ai_agent(script_structure_prompt, "Script Structure Generation")
    ...
    production_prompt = build_production_prompt(data, kb, concept_text, script_structure_text, args.chapter, old_script)
    final_script_text = call_ai_agent(production_prompt, "Final Asset Generation")
