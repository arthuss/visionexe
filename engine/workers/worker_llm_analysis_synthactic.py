...
def load_morphology_tokens(segment_dir):
...
def write_analysis(target_dir, content):
    target_path = os.path.join(target_dir, "analysis_llm_synthactic.txt")
...
def build_prompt(text_content, phase_limit, tokens):
    if tokens:
        input_block = (
            "Input JSON:\n"
            f"{json.dumps({'tokens': tokens}, ensure_ascii=False, indent=2)}"
        )
        context_block = f"\n\nSource Text:\n{text_content}"
    else:
        input_block = f"Input Text:\n{text_content}"
        context_block = ""
    return f"""Instruction: Perform a scientific Syntactic Analysis (Level C) of the Ge'ez text.
Goal: Enumerate the full structure space without choosing a single parse.

SCIENTIFIC RULES:
1. NO AMBIGUITY REDUCTION: list all valid parses (construct chain vs apposition, verbal vs nominal).
2. TOKEN FIDELITY: use only the surface tokens as provided; do not invent or split tokens.
3. FORMAL NOTATION: use bracket notation and explicit head-dependent relations.
4. NO WORLD KNOWLEDGE: syntax only, no semantic preference.
...
def build_batch_prompt(story_text, segments):
    ...
    return (
        "Instruction: Perform a scientific Syntactic Analysis (Level C) per segment.\n"
        "Goal: Enumerate the full structure space without choosing a single parse.\n\n"
        "Rules:\n"
...
def collect_segment_inputs(chapter_id, chapter_name, chapter_dir, segment_label, completed, target_chapters, include_wave):
    ...
    for segment_name, segment_dir in iter_segments(chapter_dir, segment_label):
        ...
        if key in completed and not target_chapters:
            continue
        wait_for_dependency(
            os.path.join(segment_dir, MORPHOLOGY_ANALYSIS_FILENAME),
            "morphologic",
        )
        ...
        tokens = load_morphology_tokens(segment_dir) or tokenize_text(text_content)
        segments.append({
...
def main():
    ...
    if not progress_csv:
        progress_csv = str(Path(data_root) / "analysis" / "analysis_progress_python_synthactic_v2.csv")
    else:
    ...
    for chapter_name, chapter_num in chapter_entries:
        ...
        if args.chapter_batch:
            ...
            for item in payload.get("segments", []):
                ...
                if not segment_label_value or not syntax:
                    continue
                segment_info = segment_map.get(segment_label_value)
                if not segment_info:
                    continue
                content = json.dumps({"syntax": syntax}, ensure_ascii=False, indent=2)
        ...
        if args.per_segment:
            ...
            for segment_name, segment_dir in segment_entries:
                ...
                tokens = load_morphology_tokens(segment_dir) or tokenize_text(text_content)
                prompt = build_prompt(text_content, phase_limit, tokens)
                start_time = time.time()
                if use_vertex:
                    result = call_vertex_gemini(
                        prompt,
            ...
            wait_for_dependency(
                os.path.join(chapter_dir, MORPHOLOGY_ANALYSIS_FILENAME),
                "morphologic",
            )
            ...
            tokens = load_morphology_tokens(chapter_dir) or tokenize_text(text_content)
            prompt = build_prompt(text_content, phase_limit, tokens)
            start_time = time.time()
            if use_vertex:
                result = call_vertex_gemini(
