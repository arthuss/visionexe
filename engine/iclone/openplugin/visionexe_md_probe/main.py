...
def main():
    _add_repo_root()
    from engine.iclone import md_probe
...
def initialize_plugin():
    try:
        _add_menu_action("Run VisionExe MD Probe", "visionexe_md_probe_action", run_script)
    except Exception as exc:  # pylint: disable=broad-except
        try:
            import RLPy

            RLPy.RUi.ShowMessageBox(str(exc), "VisionExe MD Probe", RLPy.EMsgButton_Ok)
        except Exception:
            print(f"[VisionExe MD Probe] {exc}")
